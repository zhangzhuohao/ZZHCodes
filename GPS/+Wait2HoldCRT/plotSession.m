...
%% get behavior index
ind_portL       = obj.PortCorrect==1;
ind_portR       = obj.PortCorrect==2;

ind_short       = obj.Stage==1 & obj.FP==0.5;
ind_med         = obj.Stage==1 & obj.FP==1;
ind_long        = obj.Stage==1 & obj.FP==1.5;

ind_correct     = strcmp(obj.Outcome, 'Correct');
ind_correctL    = ind_portL & ind_correct;
ind_correctR    = ind_portR & ind_correct;

ind_wrong       = strcmp(obj.Outcome, 'Wrong');
ind_wrongL      = ind_portL & ind_wrong;
ind_wrongR      = ind_portR & ind_wrong;

ind_late        = (strcmp(obj.Outcome, 'LateCorrect') | strcmp(obj.Outcome, 'LateWrong'));
...
text(8.6, 3, 'Init', 'FontWeight','bold', 'HorizontalAlignment', 'center');
viscircles([3.5, 3], 0.3,  'color', 'k', 'LineWidth', 1);
text(3.9, 3, 'Cent', 'FontWeight', 'bold');

viscircles([3, 1.5], 0.3, 'color', Color.PortL);
text(1.2, 1.5, 'Left', 'FontWeight','bold', 'HorizontalAlignment', 'left', 'Color', Color.PortL);
viscircles([3, 4.5], 0.3,  'color', Color.PortR);
text(1.2, 4.5, 'Right', 'FontWeight', 'bold', 'HorizontalAlignment', 'left', 'Color', Color.PortR);
...
while CountStart+WinSize < obj.NumTrials
    ...
    LateRatio = [LateRatio 100*sum(strcmp(thisOutcome, 'LateCorrect') | strcmp(thisOutcome, 'LateWrong')) / length(thisOutcome)];
...
plot(WinPos, LateRatio, 'o', 'linestyle', '-', 'color', Color.Late, ...
    'markersize', 5, 'linewidth', 1, 'markerfacecolor', Color.Late, 'markeredgecolor', 'w');
...
set(ha6, 'units', 'centimeters', 'position', [1.5+plotsize1(1)+1 16-plotsize1(2)-1, plotsize1], 'nextplot', 'add', ...
    'ylim', [0 2.5], 'xlim', [0 3], 'yscale', 'linear','ticklength', [0.01 0.1], 'fontsize', 8)
xlabel('Time in session (s)')
ylabel('Hold duration (s)')
set(ha6, 'xlim', [0 center_pokes(end)+5]);

line([center_pokes center_pokes]', [0 2.5/20], 'color', 'K')
...
% late
scatter(center_pokes(ind_lateL | ind_lateR), ...
    obj.HoldDuration(ind_lateL | ind_lateR), ...
    22*(obj.FP(ind_lateL | ind_lateR)+0.5), Color.Late, choice_symbols{2}, 'Markerfacealpha', 0.8, 'linewidth', 1.5);

%% plot hold duration density
ha7 = axes;
set(ha7, 'units', 'centimeters', 'position', [1.5+2*plotsize1(1)+1.5 16-plotsize1(2)-1, plotsize2], 'nextplot', 'add', 'ylim', [0 2.5], ...
...
set(ha8, 'units', 'centimeters', 'position', [1.5+plotsize1(1)+1 16-2*plotsize1(2)-2, plotsize1], 'nextplot', 'add', ...
    'ylim', [0 .5], 'xlim', [0 3], 'yscale', 'linear','ticklength', [0.01 0.1], 'fontsize', 8)
xlabel('Time in session (s)')
ylabel('Reaction time (s)')
set(ha8, 'xlim', [0 center_pokes(end)+5]);

line([center_pokes center_pokes]', [0 0.5/20], 'color', 'K')
yline(.5, 'Color', [.7 .7 .7], 'LineWidth', 1.5, 'LineStyle', '--');
...
ha9 = axes;
set(ha9, 'units', 'centimeters', 'position', [1.5+2*plotsize1(1)+1.5 16-2*plotsize1(2)-2, plotsize2], 'nextplot', 'add', 'ylim', [0 .5], ...
    'xlim', [0 1], 'yscale', 'linear', 'yticklabel', [], 'ticklength', [0.02 0.1], 'fontsize', 8)
...
line([center_pokes center_pokes]', [0 0.1], 'color', 'k')
...
ha11 = axes;
set(ha11, 'units', 'centimeters', 'position', [1.5+2*plotsize1(1)+1.5 16-3*plotsize1(2)-3, plotsize2], 'nextplot', 'add', 'ylim', [0 2], ...
    'xlim', [0 1], 'yscale', 'linear', 'yticklabel', [], 'ticklength', [0.02 0.1], 'fontsize', 8)
...
ha12 = axes;
set(ha12,'units', 'centimeters', 'position', [1.5, 16-3*plotsize1(2)-3, plotsize3], 'nextplot', 'add', 'ylim', [0 100], 'xlim', [0.5 10.5], ...
    'xtick', [1 2.5 3.5 5 6 7.5 8.5 10], 'xticklabel', {'L', 'R', '', '', '', ''}, 'fontsize', 8, 'ticklength', [0.01 0.1])
ylabel('Performance (%)');
...
line([8.5 10], [obj.Performance.LateRatio(length(obj.MixedFP)) obj.Performance.LateRatio(2*length(obj.MixedFP)+1)], 'color', Color.Late, 'linewidth', 1.2);
...
line([.5 1], [1 1], 'color', Color.Late, 'linewidth', 1.2);

text(1.2, 4, 'Correct', 'FontSize', 7)
text(1.2, 3, 'Wrong', 'FontSize', 7)
text(1.2, 2, 'Premature', 'FontSize', 7)
text(1.2, 1, 'Late', 'FontSize', 7)

%% Reaction time to each port
ha13 = axes; % this is the legend
set(ha13,'units', 'centimeters', 'position', [1.5+plotsize3(1)+1, 16-3*plotsize1(2)-3, plotsize3], 'nextplot', 'add', 'ylim', [0 .5], 'xlim', [0 3], ...
