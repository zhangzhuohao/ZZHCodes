classdef Violin < handle
    ...
    properties (Dependent=true)
        ...
        ScatterSize     % data size for the scatter plot
    ...
    methods
        function obj = Violin(data, pos, varargin)
            ...
            obj.ScatterPlot = ...
                scatter(pos + jitter.*jitterstrength, data, args.ScatterSize, 'filled');
            ...
            if ~isempty(data2)
                ...
                obj.ScatterPlot2 = ...
                    scatter(pos + jitter.*jitterstrength, data2, args.ScatterSize, 'filled');
            end
        ...
        %% set graphical preferences
            ...
            obj.ScatterSize = args.ScatterSize;
        ...
        function set.ViolinAlpha(obj, alpha)
            ...
            obj.ScatterPlot.MarkerFaceAlpha = min(2*alpha{1}, 1);
            if ~isempty(obj.ViolinPlot2)
                obj.ViolinPlot2.FaceAlpha = alpha{2};
                obj.ScatterPlot2.MarkerFaceAlpha = min(2*alpha{1}, 1);
            end
        end
        ...
        function set.ScatterSize(obj, size)
            obj.ScatterPlot.SizeData = size;
            if ~isempty(obj.ScatterPlot2)
                obj.ScatterPlot2.SizeData = size;
            end
        end

        function size = get.ScatterSize(obj)
            if ~isempty(obj.ScatterPlot)
                size = obj.ScatterPlot.SizeData;
            end
        end
    ...
    methods (Access=private)
        function results = checkInputs(~, data, pos, varargin)
            ...
            p.addParameter('ScatterSize', 8, @isnumeric);
