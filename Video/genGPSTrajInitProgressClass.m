...
for d = 1:length(Folders)
    ...
    Files = get_mat_files(ProtocolDir, "FileType", 'TrajInitSessionClass');
    if isempty(Files)
        continue
    end
    ...
    %
    TrajProgressClass = GPSTrajInitProgressClass(TrajSessionClassAll, ProtocolDir, BehProgressClass.BehavTable);
