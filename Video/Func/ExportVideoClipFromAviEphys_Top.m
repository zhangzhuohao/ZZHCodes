function ExportVideoClipFromAviEphys_Top(FrameTable, ClipInfo, scn_scale, remake, view, x_rev)
...
%% get information
path_info = split(pwd, filesep);
Subject = path_info{end-1};
Session = path_info{end};

% load Ephys data
r_file = sprintf('./RTarray_%s_%s.mat', Subject, Session);
if exist(r_file, 'file')
    load(r_file, 'r');
else
    error("No RTarray file!");
end
