...
for i = 1:length(SessionFolders)
    tracking_file = dir(fullfile(SessionFolders(i), "*DLCTrackingOut_Init*.mat"));
...
for i = 1:length(Folders)
    ...
    tracking_file = dir(fullfile(Folders(i), "*DLCTrackingOut_Init*.mat"));
    ...
    TrajSessionClass = GPSTrajInitSessionClass(fullfile(Folders(i), tracking_file.name), fullfile(Folders(i), behclass_file.name));
